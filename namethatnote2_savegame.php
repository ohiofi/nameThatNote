...
if (isset($_SESSION['MM_Username'])) {
  $colname_rsGame10 = $_SESSION['MM_Username'];
}
mysql_select_db($database_ohiofi, $ohiofi);// *** Get the number of correct answers
$query_rsGame10=sprintf("SELECT game10responses.entryNumber, game10responses.questionNumber FROM game10responses WHERE userName=%s AND game10responses.tally=1 ORDER BY game10responses.questionNumber DESC",
  GetSQLValueString($colname_rsGame10, "text")); 
$rsGame10 = mysql_query($query_rsGame10, $ohiofi) or die(mysql_error());
$row_rsGame10 = mysql_fetch_assoc($rsGame10);
$totalRows_rsGame10 = mysql_num_rows($rsGame10);// *** totalRows_rsGame10 is the number of correct answers

mysql_select_db($database_ohiofi, $ohiofi);// *** Get the total number of answers
$query_rsScore3=sprintf("SELECT game10responses.entryNumber, game10responses.questionNumber FROM game10responses WHERE userName=%s",
  GetSQLValueString($colname_rsGame10, "text")); 
...
$query_rsUser=sprintf("SELECT users.userID, users.game10 FROM users WHERE userName=%s ",
  GetSQLValueString($colname_rsGame10, "text")); 
$rsUser = mysql_query($query_rsUser, $ohiofi) or die(mysql_error());
$row_rsUser = mysql_fetch_assoc($rsUser);
$totalRows_rsUser = mysql_num_rows($rsUser);

$points = $_GET["p"] + $totalRows_rsGame10;
...
$tally=0;
$game10total=$totalRows_rsScore3+count($questions);
...
for ($i = 0; $i < count($questions); $i++){
	 ...
	 $insertSQL = sprintf("INSERT INTO game10responses (entryNumber, userName, questionNumber, question, answer, tally) VALUES ('$entryNumber','$userName','$tempQuestionNumber', '$tempQuestion', '$tempAnswer', '$tally')");
					   
	  		mysql_select_db($database_ohiofi, $ohiofi);
	 		$Result1 = mysql_query($insertSQL, $ohiofi) or die(mysql_error());
}

$updateSQL = sprintf("UPDATE users SET game10=%s, game10total=%s WHERE userID=%s",// *** Update the "users" table
				   GetSQLValueString($points, "int"),
				   GetSQLValueString($game10total, "int"),
				   GetSQLValueString($userNumber, "int"));

mysql_select_db($database_ohiofi, $ohiofi);
$Result1 = mysql_query($updateSQL, $ohiofi) or die(mysql_error());
$goHere="namethatnote2.php";
