...
if (isset($_SESSION['MM_Username'])) {
  $colname_rsGame11 = $_SESSION['MM_Username'];
}
mysql_select_db($database_ohiofi, $ohiofi);// *** Get the number of correct answers
$query_rsGame11=sprintf("SELECT game11responses.entryNumber, game11responses.questionNumber FROM game11responses WHERE userName=%s AND game11responses.tally=1 ORDER BY game11responses.questionNumber DESC",
  GetSQLValueString($colname_rsGame11, "text")); 
$rsGame11 = mysql_query($query_rsGame11, $ohiofi) or die(mysql_error());
$row_rsGame11 = mysql_fetch_assoc($rsGame11);
$totalRows_rsGame11 = mysql_num_rows($rsGame11);// *** totalRows_rsGame11 is the number of correct answers

mysql_select_db($database_ohiofi, $ohiofi);// *** Get the total number of answers
$query_rsScore3=sprintf("SELECT game11responses.entryNumber, game11responses.questionNumber FROM game11responses WHERE userName=%s",
  GetSQLValueString($colname_rsGame11, "text")); 
...
$query_rsUser=sprintf("SELECT users.userID, users.game11 FROM users WHERE userName=%s ",
  GetSQLValueString($colname_rsGame11, "text")); 
$rsUser = mysql_query($query_rsUser, $ohiofi) or die(mysql_error());
$row_rsUser = mysql_fetch_assoc($rsUser);
$totalRows_rsUser = mysql_num_rows($rsUser);

$points = $_GET["p"] + $totalRows_rsGame11;
...
$tally=0;
$game11total=$totalRows_rsScore3+count($questions);
...
for ($i = 0; $i < count($questions); $i++){
	 ...
	 $insertSQL = sprintf("INSERT INTO game11responses (entryNumber, userName, questionNumber, question, answer, tally) VALUES ('$entryNumber','$userName','$tempQuestionNumber', '$tempQuestion', '$tempAnswer', '$tally')");
					   
	  		mysql_select_db($database_ohiofi, $ohiofi);
	 		$Result1 = mysql_query($insertSQL, $ohiofi) or die(mysql_error());
}

$updateSQL = sprintf("UPDATE users SET game11=%s, game11total=%s WHERE userID=%s",// *** Update the "users" table
				   GetSQLValueString($points, "int"),
				   GetSQLValueString($game11total, "int"),
				   GetSQLValueString($userNumber, "int"));

mysql_select_db($database_ohiofi, $ohiofi);
$Result1 = mysql_query($updateSQL, $ohiofi) or die(mysql_error());
$goHere="namethatnotebass.php";
