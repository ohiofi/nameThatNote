...
$query_rsScoreboard = "SELECT userName, game10 FROM users ORDER BY game10 DESC";
...
mysql_select_db($database_ohiofi, $ohiofi);
$query_rsScoreboard = "SELECT userName, game10 FROM users WHERE game10 > 0 ORDER BY game10 DESC";
$query_limit_rsScoreboard = sprintf("%s LIMIT %d, %d", $query_rsScoreboard, $startRow_rsScoreboard, $maxRows_rsScoreboard);
$rsScoreboard = mysql_query($query_limit_rsScoreboard, $ohiofi) or die(mysql_error());
$row_rsScoreboard = mysql_fetch_assoc($rsScoreboard);
...
<title>Name That Note web app</title>
...
<div id="main">
  <div id="header">
    <h1>Name That Note, Treble 2</h1>
  </div>
  <div class="tutorsplitter">
  <div id="video">
  		<img src="img/namethatnotetrebletutor.png" width="450px"/>
  	</div>
  <p>How to play: Click on the letter that corresponds with the note on the staff.</p>
  </div>
  <div class="tutorsplitter">
  ...
  <td style="text-align:right;"><b><i>High Scores</i></b></td>
	...
	do { ?>
      <tr>
        ...
        <td><?php echo " "; ?></td>
        <td><?php echo " ",$row_rsScoreboard['game10'];
		if ($row_rsScoreboard['game10'] != 1)
			echo " pts";
		else
			echo " pt";
		?>
  ...
  <div id="gobackorcontinue" class="centered">
    ...
    <a href="namethatnote2.php"><div class="button gobackorcontinue">
